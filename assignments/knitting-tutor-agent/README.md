...
모델은 사용자의 한 문장을 키워드로만 분류하지 않고 현재 대화, 선택한 작품, 진행 중인 단계를 함께
판단합니다. 답변은 질문에 직접 답하는 것을 우선하며, 아래 서비스 자료 중 지금 도움이 되는 최소한의
모듈만 덧붙입니다. 같은 전체 도안이나 준비물, 기법 카드를 매 대화마다 반복하지 않습니다.

| 사용자 의도 | 기본 응답 | 필요할 때 연결하는 구조화 자료 |
| --- | --- | --- |
| 뜨개 개념·일반 질문 | 질문에 바로 설명 | 관련 기법 1개 또는 도구 1개 |
| 기법을 배우거나 막힌 동작 질문 | 동작과 확인 포인트 설명 | 검수된 기법 카드, 이미지·영상, 짧은 연습 |
| 도구 선택·사용 질문 | 용도와 고르는 기준 설명 | 해당 도구의 이미지와 상세 가이드 |
| 첫 작품 추천·비교 | 수준과 목표에 맞는 이유 설명 | 검수된 작품 최대 3개, 선택 전에는 진도 시작 안 함 |
| 특정 작품 시작 | 전체 구조와 준비 순서 안내 | 작품 전체 도안, 준비물, 필요한 기법, 1단계 |
| 현재 단계 설명 | 현재 단만 자세히 안내 | 그 단계에서 새로 쓰는 기법만 연결 |
| 다음 단계·완료 보고 | 현재 진도를 한 번만 변경 | 변경된 다음 단계의 설명과 확인 기준 |
| 사진 속 문제·완성작 질문 | 사진에서 확인되는 사실부터 답변 | 문제 진단 카드, 꼭 필요할 때만 추가 사진 요청 |
| 도안·약어 질문 | 기호·영문 도안과 초보 설명을 함께 제공 | 도안 해설 및 반복 구간 안내 |
| 뜨개와 무관한 질문 | 짧게 범위를 알리고 뜨개 질문으로 유도 | 진행 중인 작품과 진도는 그대로 보존 |

대화 모델은 `support_modules`, `technique_names`, `tool_slugs`, `suggested_curriculum_ids`를 구조화해
반환합니다. 애플리케이션은 이 선택을 검수된 `content/` 카탈로그와 curriculum에 연결합니다. 작품의
코 수·단 수와 진도 변경은 모델이 새로 만들지 않고 저장된 데이터와 결정론적 상태 전이로 처리합니다.
